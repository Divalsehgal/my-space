...
export default function BlogPageContent({ posts }: Readonly<BlogPageContentProps>) {
    ...
    const renderCarouselItem = useCallback((post: ContentfulPost) => <BlogCard post={post} />, []);
                ...
                {posts.length === 0 ? (
                    <div className={styles["blogs-empty-state"]}>
                        ...
                        <p className={styles["blogs-empty-message"]}>
                            No posts found. Make sure your Contentful space is connected and has published posts.
                        </p>
                    </div>
                ) : (
                    <>
